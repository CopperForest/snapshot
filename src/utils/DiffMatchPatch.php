...
// Shorter text is inside the longer text (speedup)
            $diffs = array(
                ...
                array( DIFF_INSERT, mb_substr( $longtext,
                            $i + mb_strlen( $shorttext ) ) )
            );
...
// Delete the offending records and add the merged ones.
                            $a = $this->diff_main( $text_delete, $text_insert,
                                    false );
                            array_splice( $diffs,
...
// Walk the front path one step.
            ...
            for ( $k = -$d; $k <= $d; $k += 2 ) {
                ...
                while ( !$done && ($x < $text1_length) && ($y < $text2_length) && (mb_substr( $text1,
                        $x, 1 ) == mb_substr( $text2, $y, 1 )) ) {
                    $x++;
                    $y++;
                    if ( $doubleEnd ) {
                        $footstep = $x . ',' . $y;
...
// Front path ran over reverse path.
                    ...
                    $v_map2 = array_slice( $v_map2, 0,
                            $footsteps[ $footstep ] + 1 );
                    $a = $this->diff_path1( $v_map1, mb_substr( $text1, 0, $x ),
                            mb_substr( $text2, 0, $y ) );

                    return array_merge( $a,
                            $this->diff_path2( $v_map2, mb_substr( $text1, $x ),
                                    mb_substr( $text2, $y ) ) );
                }
            }
...
// Walk the reverse path one step.
                ...
                for ( $k = -$d; $k <= $d; $k += 2 ) {
                    ...
                    while ( !$done && $x < $text1_length && $y < $text2_length && mb_substr( $text1,
                            $text1_length - $x - 1, 1 ) == mb_substr( $text2,
                            $text2_length - $y - 1, 1 ) ) {
                        $x++;
                        $y++;
                        $footstep = ($text1_length - $x) . ',' . ($text2_length - $y);
...
// Reverse path ran over front path.
                        $v_map1 = array_slice( $v_map1, 0,
                                $footsteps[ $footstep ] + 1 );
                        $a = $this->diff_path1( $v_map1,
                                mb_substr( $text1, 0, $text1_length - $x ),
                                mb_substr( $text2, 0, $text2_length - $y ) );
                        return array_merge( $a,
                                $this->diff_path2( $v_map2,
                                        mb_substr( $text1, $text1_length - $x ),
                                        mb_substr( $text2, $text2_length - $y ) ) );
                    }
                }
            }
        }
...
// First check if the second quarter is the seed for a half-match.
        $hm1 = $this->diff_halfMatchI( $longtext, $shorttext,
                ceil( mb_strlen( $longtext ) / 4 ) );
// Check again based on the third quarter.
        $hm2 = $this->diff_halfMatchI( $longtext, $shorttext,
                ceil( mb_strlen( $longtext ) / 2 ) );
...
// Start with a 1/4 length mb_substring at position i as a seed.
        ...
        while ( ($j = mb_strpos( $shorttext, $seed, $j + 1 )) !== false ) {
            $prefixLength = $this->diff_commonPrefix( mb_substr( $longtext, $i ),
                    mb_substr( $shorttext, $j ) );
            $suffixLength = $this->diff_commonSuffix( mb_substr( $longtext, 0,
                            $i ), mb_substr( $shorttext, 0, $j ) );
            if ( mb_strlen( $best_common ) < $suffixLength + $prefixLength ) {
                $best_common = mb_substr( $shorttext, $j - $suffixLength,
                                $suffixLength ) . mb_substr( $shorttext, $j,
                                $prefixLength );
...
// Duplicate record
                    $zzz_diffs = array_splice( $diffs,
                            $equalities[ $equalitiesLength - 1 ], 0,
                            array( array(
                            DIFF_DELETE,
                            $lastequality
                        ) ) );
...
// First, shift the edit as far left as possible.
                ...
                if ( $commonOffset !== '' ) {
                    $commonString = mb_substr( $edit,
                            mb_strlen( $edit ) - $commonOffset );
                    $equality1 = mb_substr( $equality1, 0,
                            mb_strlen( $equality1 ) - $commonOffset );
                    $edit = $commonString . mb_substr( $edit, 0,
                                    mb_strlen( $edit ) - $commonOffset );
...
// Second, step character by character right, looking for the best fit.
                ...
                $bestScore = $this->diff_cleanupSemanticScore( $equality1, $edit ) + $this->diff_cleanupSemanticScore( $edit,
                                $equality2 );
                while ( isset( $equality2[ 0 ] ) && $edit[ 0 ] === $equality2[ 0 ] ) {
                    $equality1 .= $edit[ 0 ];
                    $edit = mb_substr( $edit, 1 ) . $equality2[ 0 ];
                    $equality2 = mb_substr( $equality2, 1 );
                    $score = $this->diff_cleanupSemanticScore( $equality1, $edit ) + $this->diff_cleanupSemanticScore( $edit,
                                    $equality2 );
...
// One point for non-alphanumeric.
        if ( preg_match( $punctuation, $one[ mb_strlen( $one ) - 1 ] ) || preg_match( $punctuation,
                        $two[ 0 ] ) ) {
            $score++;
// Two points for whitespace.
            if ( preg_match( $whitespace, $one[ mb_strlen( $one ) - 1 ] ) || preg_match( $whitespace,
                            $two[ 0 ] ) ) {
                $score++;
// Three points for line breaks.
                if ( preg_match( $linebreak, $one[ mb_strlen( $one ) - 1 ] ) || preg_match( $linebreak,
                                $two[ 0 ] ) ) {
                    $score++;
// Four points for blank lines.
                    if ( preg_match( $blanklineEnd, $one ) || preg_match( $blanklineStart,
                                    $two ) ) {
                        $score++;
                    }
                }
                            ...
                            if ( $commonlength !== 0 ) {
                                if ( ($pointer - $count_delete - $count_insert) > 0 && $diffs[ $pointer - $count_delete - $count_insert - 1 ][ 0 ] == DIFF_EQUAL ) {
                                    ...
                                    array_splice( $diffs, 0, 0,
                                            array( array(
                                            DIFF_EQUAL,
                                            mb_substr( $text_insert, 0,
                                                    $commonlength )
                                ) ) );
                                    $pointer++;
                                }
                                $text_insert = mb_substr( $text_insert,
                                        $commonlength );
                                $text_delete = mb_substr( $text_delete,
                                        $commonlength );
                            }
// Factor out any common suffixies.
                            ...
                            if ( $commonlength !== 0 ) {
                                ...
                                $text_insert = mb_substr( $text_insert, 0,
                                        mb_strlen( $text_insert ) - $commonlength );
                                $text_delete = mb_substr( $text_delete, 0,
                                        mb_strlen( $text_delete ) - $commonlength );
                            }
                        }
// Delete the offending records and add the merged ones.
                        if ( $count_delete === 0 ) {
...
// Shift the edit over the previous equality.
                    $diffs[ $pointer ][ 1 ] = $diffs[ $pointer - 1 ][ 1 ] . mb_substr( $diffs[ $pointer ][ 1 ],
                                    0,
...
// Is there a nearby exact match? (speedup)
        ...
        if ( $best_loc !== false ) {
            $score_threshold = min( $this->match_bitapScore( 0, $best_loc,
                            $pattern, $loc ), $score_threshold );
        }

// What about in the other direction? (speedup)
        $best_loc = mb_strrpos( $text, $pattern,
                min( $loc + mb_strlen( $pattern ), mb_strlen( $text ) ) );
        if ( $best_loc !== false ) {
            $score_threshold = min( $this->match_bitapScore( 0, $best_loc,
                            $pattern, $loc ), $score_threshold );
        }
...
// error level.
            ...
            while ( $bin_min < $bin_mid ) {
                if ( $this->match_bitapScore( $d, $loc + $bin_mid, $pattern,
                                $loc ) <= $score_threshold ) {
                    $bin_min = $bin_mid;
                } else {
                    $bin_max = $bin_mid;
    ...
    function patch_addContext( $patch, $text )
    {
        ...
        && mb_strlen( $pattern ) < Match_MaxBits - $this->Patch_Margin - $this->Patch_Margin ) {
            ...
            $pattern = mb_substr( $text, max( $patch->start2 - $padding, 0 ),
                    ($patch->start2 + $patch->length1 + $padding) - max( $patch->start2 - $padding,
                            0 ) );
        }
...
// A new patch starts here.
            ...
            switch ( $diff_type ) {
                case DIFF_INSERT :
                    ...
                    $postpatch_text = mb_substr( $postpatch_text, 0,
                                    $char_count2 ) . $diff_text . mb_substr( $postpatch_text,
                                    $char_count2 );
                    break;
                case DIFF_DELETE :
                    ...
                    $patch->diffs[ $patchDiffLength++ ] = $diffs[ $x ];
                    $postpatch_text = mb_substr( $postpatch_text, 0,
                                    $char_count2 ) . mb_substr( $postpatch_text,
...
// a monster delete.
                $start_loc = $this->match_main( $text,
                        mb_substr( $text1, 0, Match_MaxBits ), $expected_loc );
                if ( $start_loc != -1 ) {
                    $end_loc = $this->match_main( $text,
                            mb_substr( $text1,
                                    mb_strlen( $text1 ) - Match_MaxBits ),
                            $expected_loc + mb_strlen( $text1 ) - Match_MaxBits );
...
// Found a match. :)
                ...
                if ( $end_loc == -1 ) {
                    ...
                    $text2 = mb_substr( $text, $start_loc,
                            $end_loc + Match_MaxBits - $start_loc );
                }
                if ( $text1 == $text2 ) {
// Perfect match, just shove the replacement text in.
                    $text = mb_substr( $text, 0, $start_loc ) . $this->diff_text2( $patches[ $x ]->diffs ) . mb_substr( $text,
                                    $start_loc + mb_strlen( $text1 ) );
                } else {
                        ...
                        for ( $y = 0; $y < count( $patches[ $x ]->diffs ); $y++ ) {
                            ...
                            if ( $mod[ 0 ] === DIFF_INSERT ) { // Insertion
                                $text = mb_substr( $text, 0,
                                                $start_loc + $index2 ) . $mod[ 1 ] . mb_substr( $text,
                                                $start_loc + $index2 );
                            } elseif ( $mod[ 0 ] === DIFF_DELETE ) { // Deletion
                                $text = mb_substr( $text, 0,
                                                $start_loc + $index2 ) . mb_substr( $text,
                                                $start_loc + $this->diff_xIndex( $diffs,
...
// Strip the padding off.
        $text = mb_substr( $text, mb_strlen( $nullPadding ),
...
// Grow first equality.
            ...
            $diffs[ 0 ][ 1 ] = mb_substr( $nullPadding,
                            mb_strlen( $diffs[ 0 ][ 1 ] ) ) . $diffs[ 0 ][ 1 ];
...
// Grow last equality.
            ...
            $diffs[ count( $diffs ) - 1 ][ 1 ] .= mb_substr( $nullPadding, 0,
                    $extraLength );
            $patch->length1 += $extraLength;
            $patch->length2 += $extraLength;
        }
...
// Deletion or equality. Only take as much as we can stomach.
                            $diff_text = mb_substr( $diff_text, 0,
                            ...
                            if ( $diff_text == $bigpatch->diffs[ 0 ][ 1 ] ) {
                                ...
                                $bigpatch->diffs[ 0 ][ 1 ] = mb_substr( $bigpatch->diffs[ 0 ][ 1 ],
                                        mb_strlen( $diff_text ) );
                            }
                        }
                    }
// Compute the head context for the next patch.
                    ...
                    $precontext = mb_substr( $precontext,
                            mb_strlen( $precontext ) - $this->Patch_Margin );
// Append the end context for this patch.
                    $postcontext = mb_substr( $this->diff_text1( $bigpatch->diffs ),
                            0, $this->Patch_Margin );
                    if ( $postcontext !== '' ) {
...
function mb_chr( $num )
{
    return mb_convert_encoding( '&#' . intval( $num ) . ';', 'UTF-8',
            'HTML-ENTITIES' );
}
